...
class Club
{
	...
	{
				$db = Db::getInstance();
				$sql = "SELECT * FROM ".$this->tabla." WHERE asociacion = :id";
    			$bind = array(
        		':id' => $asociacion
    			);
	...
	public function saveDirectorio ($id, $cargo, $nombre, $run, $fecha) {
		$db = Db::getInstance();
		$data = array(
			'tipo' => $this->tipoDirectorio,
			'ooddId' => $id,
		...
		$db->save('com_oodd_directorio_pdf', $data, "tipo=:tipo AND cargo = :cargo AND prox_eleccion=:prox_eleccion AND ooddId=:ooddId" , array(
			'tipo' => $this->tipoDirectorio,
			'cargo' => $cargo,
			'prox_eleccion' => $fecha,
			'ooddId' => $id
