...
class Disciplina
{
	...
	public static function getAllEspecialidades(){
		$db2 = Null;
        $db2 = Db::getInstance();
        $sql2 = "SELECT * FROM com_especialidades";
        ...
        if ($cont == 0) {
            ...
            $db1 = Null;
            $db1 = Db::getInstance();
            $row_p1 = $db1->fetchAll($sql2);
            return $row_p1;
        }
	}

	public static function getJuecesByDisciplina($disciplina=''){
		$db2 = Null;
        $db2 = Db::getInstance();
        $sql2 = "SELECT nombre,apellido,id,rut FROM com_jueces";
		$bind = [];

		if($disciplina){
			$sql2.=' WHERE disciplina=:disciplina';
			$bind = array(
