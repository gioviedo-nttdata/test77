...
class Usuario
{
			...
			$data = array(
				...
				if ($roles['entrenador'] == '1') {
				   ...
				   foreach ($datosN['disciplina'] as $disci) {
					   $this->asignarDisciplina($this->id, $disci, $datosN['tipo']);    
                   }
				   ...
				   foreach ($datosN['cargo'] as $carg) {
                    ...
                    $this->asignarDisciplina($this->id, $datosN['disciplina'], $datosN['tipo']);    
                }
	...
	public function getAll ($paginado=1, $tipo='todos', $disciplina, $tipoLimit='', $externo = 0, $opciones = array(),$club='0')
	{
				...
				$sql = "SELECT ".$this->tabla.".*, IF(fecnac_pdf IS NULL, 0, 1) AS up_certificado_nacimiento, IF(fecnac = fecnac_pdf, 1, 0) AS fecnac_validada, com_especialidades.especialidad FROM ".$this->tabla. " "
                                ...
                                if (!empty($opciones['anoD'])) {
                                    $sql .= " AND YEAR(".$this->tabla.".fecnac) <= :anoD";
                                    $bind[":anoD"] = $opciones['anoD'];
                                }
        ...
        public function checkemail ($rut)
	{
        ...
        public function updateFechaNacimientoPdf($id,$fecha){
            $db = Db::getInstance();
			$data = array(
                            'fecnac_pdf' => $fecha
                        );
        ...
        public function extraerYActualizarFecNacPdf($file, $idUser){
            $pdfParser = new PdfParserController($file);
            $patronFechaNacimiento = "/fecha nacimiento\s*:.*?(\d{1,2}\s\w+\s\d{4})/";
            preg_match($patronFechaNacimiento, strtolower(preg_replace('/\s+/', ' ', $pdfParser->text)), $coincidencias);
            $fecha_nacimiento = $coincidencias[1] ?? "";
            $fecha_nacimiento = explode(" ",trim($fecha_nacimiento));
            $anio = $fecha_nacimiento[2] ?? "";
            $mes = Funciones::numMonthByname($fecha_nacimiento[1] ?? "");
            $dia = $fecha_nacimiento[0] ?? "";
            $fecha_nacimiento = DateTime::createFromFormat('Y-m-d', "$anio-$mes-$dia");
            if($fecha_nacimiento){
                $this->updateFechaNacimientoPdf($idUser,$fecha_nacimiento->format('Y-m-d'));
            }
        }

        public function fecNacPdfInit(){
            if(!file_exists("uploads/fecnac_pdf_init.txt")){
                $sql = "SELECT * FROM com_users_documentos WHERE ext='pdf'";
                $db = Db::getInstance();
                $docs = $db->fetchAll($sql);
                foreach($docs as $doc){
                    $idUser = $doc["user"];
                    $file = "uploads/N".$idUser."_".$doc["valor"].".pdf";
                    if(file_exists($file)){
                        $this->extraerYActualizarFecNacPdf($file,$idUser);
                    }
                }
                fopen("uploads/fecnac_pdf_init.txt", "a");
            }
        }
