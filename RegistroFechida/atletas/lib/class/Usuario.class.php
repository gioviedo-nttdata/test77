...
class Usuario
{
        ...
        public function checkemail ($rut)
	{
        ...
        public function guardarDoc($id, $valor, $nombre, $ext) {
            ...
            return $db->insert('com_users_documentos', $data);		
        }

        public function getFecNacPDF($id){
            $db = Db::getInstance();
            $sql = "SELECT fecnac_pdf FROM com_users WHERE id = :id LIMIT 1";
            $bind = array(
                        ':id' => $id
            );
            return $db->fetchAll($sql, $bind)[0]['fecnac_pdf'];
        }

        public function getImagen($id){
            $db = Db::getInstance();
            $sql = "SELECT imagen FROM com_users WHERE id = :id LIMIT 1";
            $bind = array(
                        ':id' => $id
            );
            return $db->fetchAll($sql, $bind)[0]['imagen'];
        }

        public static function getLicencia(){           
            $db1 = Db::getInstance();
            $sql = "SELECT * FROM com_licencias_users WHERE id = :id";
            $bind = array(
            ':id' => null
            );
            $res = $db1->fetchRow($sql, $bind);
            return $res;
        }
}