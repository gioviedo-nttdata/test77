...
class Authorizacion
{
    ...
    {            
		if (empty($this->login)) {
	 		header("Location: login.php?err=5");
				...
				$db = Db::getInstance();
				$sql = "SELECT * FROM com_asociaciones WHERE id = :id";
    			$bind = array(
        		':id' => $this->login
    			);
	...
	public function logIn ($email,$pass)
    {
            ...
            $pass1 = sha1(md5(trim($pass)));
				$db = Db::getInstance();
				$sql = "SELECT * FROM com_asociaciones WHERE email = :email";
    			$bind = array(
					':email' => $email
    			);
		...
		if ($cont > 0){
			...
			foreach($rowff1 as $rowff) {
          				if ($rowff['pass'] != $pass1) {
							...
							$db = Db::getInstance();
    						$db->update('com_asociaciones', $data, 'id = :id', array(':id' => $rowff['id']));
	...
	public function verificarCuenta ($id,$unique)
    {
		...
		$db = Db::getInstance();
				$sql = "SELECT * FROM com_asociaciones WHERE id = :id AND clave = :uniqueid LIMIT 1";
    			$bind = array(
        		':id' => $id,
        		':uniqueid' => $unique
    			);
				...
				//$clave00 = createhash($tokensArray,$hashLenght);
			    	...
			    	$db->update('com_asociaciones', $data, 'id = :id', array(':id' => $rowff[0]['id']));
	...
	public function modificar ($pass1,$pass2,$pass)
    {
    	...
    	//$db->insert('com_proyectos', $data);
		   ...
		   $db->update('com_asociaciones', $data, 'id = :id', array(':id' => $this->rowff['id']));
	...
	public function modificarFoto($valor)
    {
    	...
    	//$db->insert('com_proyectos', $data);
		   ...
		   $db->update('com_asociaciones', $data, 'id = :id', array(':id' => $this->id));
        ...
        public function checkrut ($rut)
	{
				//$rut = str_replace(".", "", $rut);
				$db = Db::getInstance();
				$sql = "SELECT * FROM com_asociaciones WHERE rut = :rut LIMIT 1";
    			$bind = array(
        		':rut' => $rut
    			);
