...
class Master
{
    ...
    public function asignarDisciplina($user, $disciplina)
    {
        ...
        $db->insert('com_master_disciplina', $data);
    }
    ...
    public function asignarCargo($user, $cargo)
    {
        ...
        $db->insert('com_master_cargo', $data);
    }
    ...
    public function aprobar($id, $pago)
    {
    ...
    public function getAll($paginado = 1, $tipo = 'todos', $disciplina, $tipoLimit = '', $externo = 0, $opciones = array(), $club = '0')
    {
        ...
        $sql = "SELECT " . $this->tabla . ".*, com_especialidades.especialidad, regiones.region AS regionN FROM " . $this->tabla . " "
            . "LEFT JOIN com_especialidades ON com_master.disciplina = com_especialidades.id "
            . "LEFT JOIN regiones ON com_master.region = regiones.id ";
       ...
       // $sql .= " AND " . $this->tabla . ".externo = :externo ";
    ...
    static function anoNadador($tipo)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT YEAR(com_master.fecnac) AS ano FROM com_master WHERE nadador = 1 ORDER BY fecnac" . $tipoOrden . " LIMIT 1";
    ...
    static function getAtletas($club, $disciplina)
    {
        ...
        $sql = "SELECT com_master.*, com_especialidades.especialidad FROM com_master "
            . "LEFT JOIN com_especialidades ON com_master.nadador = com_especialidades.id ";

        $sql .= "WHERE com_master.nadador = 1 AND com_master.id > :id";
        $bind = array(
            ':id' => '0'
        );
        if (!empty($disciplina)) {
            $sql .= " AND com_master.disciplina = :disciplina";
            $bind[":disciplina"] = $disciplina;
        }
        if (!empty($club)) {
            $sql .= " AND com_master.club = :club";
            $bind[":club"] = $club;
        }


        $sql .= " AND com_master.estado=0";
    ...
    static function getOneByName($nombre, $concatenador, $externo = 0)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT * FROM com_master WHERE (nadador = :nadador AND externo = :externo) AND";
    ...
    static function getAllNad($externo = 0)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT * FROM com_master WHERE nadador = :nadador AND externo=:externo ORDER BY apellido";
        $bind = array(
            ':nadador' => 1,
            ':externo' => $externo
        );
    ...
    public function getRepresentados($id)
    {
        ...
        $sql = "SELECT " . $this->tabla . ".*, com_grupos.grupo FROM " . $this->tabla . " "
            . "LEFT JOIN com_grupos ON com_master.grupo = com_grupos.id ";
    ...
    static function contarAtletasClub($club,$tipo)
	{
		...
		$db = Db::getInstance();
		$sql = "SELECT * FROM com_master WHERE ".$tipoA." = 1 AND club = :id";
		$bind = array(
			':id' => $club
		);
