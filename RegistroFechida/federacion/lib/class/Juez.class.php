...
class Juez
{
    ...
    public function asignarDisciplina($user, $disciplina)
    {
        ...
        $db->insert('com_jueces_disciplina', $data);
    }
    ...
    public function asignarCargo($user, $cargo)
    {
        ...
        $db->insert('com_jueces_cargo', $data);
    }
    ...
    public function getAll($paginado = 1, $tipo = 'todos', $disciplina, $tipoLimit = '', $externo = 0, $opciones = array(), $club = '0')
    {
        ...
        $sql = "SELECT " . $this->tabla . ".*, com_especialidades.especialidad, regiones.region AS regionN FROM " . $this->tabla . " "
            . "LEFT JOIN com_especialidades ON com_jueces.disciplina = com_especialidades.id "
            . "LEFT JOIN regiones ON com_jueces.region = regiones.id ";
    ...
    static function anoNadador($tipo)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT YEAR(com_jueces.fecnac) AS ano FROM com_jueces WHERE nadador = 1 ORDER BY fecnac" . $tipoOrden . " LIMIT 1";
    ...
    static function getAtletas($club, $disciplina)
    {
        ...
        $sql = "SELECT com_jueces.*, com_especialidades.especialidad FROM com_jueces "
            . "LEFT JOIN com_especialidades ON com_jueces.nadador = com_especialidades.id ";

        $sql .= "WHERE com_jueces.nadador = 1 AND com_jueces.id > :id";
        $bind = array(
            ':id' => '0'
        );
        if (!empty($disciplina)) {
            $sql .= " AND com_jueces.disciplina = :disciplina";
            $bind[":disciplina"] = $disciplina;
        }
        if (!empty($club)) {
            $sql .= " AND com_jueces.club = :club";
            $bind[":club"] = $club;
        }


        $sql .= " AND com_jueces.estado=0";
    ...
    static function getOneByName($nombre, $concatenador, $externo = 0)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT * FROM com_jueces WHERE (nadador = :nadador AND externo = :externo) AND";
    ...
    static function getAllNad($externo = 0)
    {
        ...
        $db = Db::getInstance();
        $sql = "SELECT * FROM com_jueces WHERE nadador = :nadador AND externo=:externo ORDER BY apellido";
        $bind = array(
            ':nadador' => 1,
            ':externo' => $externo
        );
    ...
    public function getRepresentados($id)
    {
        ...
        $sql = "SELECT " . $this->tabla . ".*, com_grupos.grupo FROM " . $this->tabla . " "
            . "LEFT JOIN com_grupos ON com_jueces.grupo = com_grupos.id ";
    ...
    static function contarAtletasClub($club,$tipo)
	{
		...
		$db = Db::getInstance();
		$sql = "SELECT * FROM com_jueces WHERE ".$tipoA." = 1 AND club = :id";
		$bind = array(
			':id' => $club
		);
