...
$users->getAll(0, 'nadador', $disciplina, '', 0, $opciones, $club);
...
//require_once 'vistas/licencias.php';
		...
		$worksheet->write(0, 6, 'Club',$format_column);
		$worksheet->write(0, 7, 'Fecha de Nacimiento',$format_column);
		$worksheet->write(0, 8, 'Licencia '.$periodo['periodo'],$format_column);
...
foreach ($users->row as $Elem) { 
			...
			$licencia = Usuario::getLicenciaByUser($Elem['id'], $periodo['periodo']);
			$pagado = "";
			if ($licencia > 0) {
				$pagado = "Licencia Pagada";				
			}
			...
			$worksheet->write($fila, 6, utf8_decode($Elem['clubN']),$format_column1);
			$worksheet->write($fila, 7, utf8_decode(date('d', $fecha)."/". date('m', $fecha)."/". date('Y', $fecha)),$format_column1);
			$worksheet->write($fila, 8, utf8_decode($pagado),$format_column1);
