	...
	public function agregar ($datosN = array())
    {
	   ...
	   if (empty($datosN['nombre'])) {
        ...
        public function getAll($id, $tipo) {
            ...
            $db2 = Null;
                                        $db2 = Db::getInstance();
                                        $sql2 = "SELECT * FROM com_jueces_documentos WHERE asociacion = :asociacion AND tipo = :tipo ORDER BY id";
                                        $bind2 = array(
                                            ':asociacion' => $id,
                                            ':tipo' => $tipo
                                        );
        ...
        public function getDocumentoOne($id) {
            ...
            $db2 = Null;
                                        $db2 = Db::getInstance();
                                        $sql2 = "SELECT * FROM com_jueces_documentos WHERE id = :id LIMIT 1";
                                        $bind2 = array(
                                            ':id' => $id
                                        );
        ...
        public function guardarDoc($id, $valor, $nombre, $ext, $tipo) {
            $db = Db::getInstance();
			$data = array(
                            'asociacion' => $id,
                            'valor' => $valor,
                ...
                $db->insert('com_jueces_documentos', $data);
		//$this->id = $db->lastInsertId();
        ...
        public function elimDocu($user, $id){
            $db = Db::getInstance();
            $db->delete('com_jueces_documentos', "id=:id AND asociacion = :user" , array(':id' => $id, ':user' => $user));
